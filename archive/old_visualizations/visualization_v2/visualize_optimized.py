...
ELEM_COLORS = {'Cu': '#E74C3C', 'Ti': '#3498DB', 'Mn': '#2ECC71', 'Nb': '#9B59B6', 'Co': '#F39C12'}
...
def plot_top10_scores():
    ...
    bars = ax.barh(range(len(labels)), top10['Comprehensive_Score'].values, color=colors)
    ...
    for idx, (_, row) in enumerate(top10.iterrows()):
        ...
        ax.scatter(row['E_hull_change'], row['BG_reduction_MP'],
                  s=200, c=color, edgecolors='black', linewidth=1, label=elem if idx < 5 else '')
        ax.annotate(f"{elem}", (row['E_hull_change'], row['BG_reduction_MP']),
    ...
    ax.set_title('带隙降幅 vs 热力学稳定性（★ MP带隙）', fontsize=14, fontweight='bold')
    ax.legend(loc='upper left')

    # 标注理想区域
    ax.annotate('理想区域\n(高降幅 + 低E_hull)', xy=(0.02, 28), fontsize=10,
